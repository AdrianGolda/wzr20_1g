...
FILE *f = fopen("vct_log.txt", "w");               // log file handle
...
long time_of_cycle, number_of_cyc;                 // variables supported fDt calculation
...
void InteractionInitialisation()
{
	...
	uni_reciv = new unicast_net( 10001);      // object for receiving messages
	uni_send = new unicast_net( 10002);       // object for sending messages
...
void VirtualWorldCycle()
{
	...
	// average time - fDt between two consequtive world cycles calculation: 
	...
	{                              
		...
		sprintf(text, "WZR-lab 2019/20 temat 1, wer. g (%0.0f fps  %0.2fms) ", fFps, 1000.0 / fFps);
		SetWindowText(main_window, text); 			
	}

	my_vehicle->Simulation(fDt);                      // simulation of own object based of previous state, forces and 
	                                                  // other actions and fDt - average time elapsed since the last simulation
	Frame frame;
	frame.state = my_vehicle->State();                // state of my own object
	frame.iID = my_vehicle->iID;                      // my object identifier

	uni_send->send((char*)&frame,"192.168.1.141", sizeof(Frame));   // sending a message to other application
}
	...
	int       nCmdShow)
{
	...
	main_window = CreateWindow(class_name, "VCT-lab 2019/20 topic 1 - Network Architectures, version g", WS_OVERLAPPEDWINDOW | WS_VISIBLE | WS_CLIPCHILDREN | WS_CLIPSIBLINGS,
		100, 50, 950, 700, NULL, NULL, hInstance, NULL);
